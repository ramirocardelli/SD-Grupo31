...
async function sendAltaAnimal(event) {
  try {
    ...
    const animalDesc = event.target.form.elements.animalDescriptionAlta.value.trim();
...
async function sendBajaAnimal(event) {
  try {
    ...
    const response = await AnimalAPIHelper.createAnimal('delete', animalData, accessToken);
...
async function sendModifAnimal(event) {
  try {
    ...
    const animalDesc = event.target.form.elements.animalDescriptionModif.value.trim();
    const accessToken = AuthStateHelper.getAccessToken();
    ...
    const response = await AnimalAPIHelper.createAnimal('patch', animalData, accessToken);
